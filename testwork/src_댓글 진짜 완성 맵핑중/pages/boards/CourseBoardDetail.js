...
const CourseBoardDetail = () => {
  ...
  // 각 답글 버튼의 text 관리
  const [replyButtonTexts, setReplyButtonTexts] = useState({})
  // 각 수정 폼 상태를 관리하는 배열
  ...
  // 답글버튼 텍스트 토글 및 상태
  const toggleReplyForm = (e, index) => {
    const currentText = e.target.innerText
    const newText = currentText === "답글" ? "취소" : "답글"
  
    // 답글 폼을 토글
    const replyForm = commentList[index].ref.current.querySelector(".replyCommentForm")
    if (newText === "취소") {
  ...
  //답글 등록
  const handleReplySubmit = (e, replyButtonText, index) => {
    e.preventDefault()
    ...
    axios.post(`/api/v1/posts/${post.id}/comments`, data)
      .then((res) => {
        ...
        // 답글 버튼이 "취소" 상태일 때만 "답글"로 변경
        if (replyButtonText === "취소") {
          setReplyButtonTexts((prev) => ({
            ...prev,
            [index]: "답글",
          }));
                  ...
                  <>
                    ...
                    <div className="flex-1">
                      <div className="commentSource">
                        ...
                        <div className="mt-2 text-sm text-gray-500">
                          ...
                          <button
                            ...
                            // ref={replyButtonRef}
                            onClick={(e) => toggleReplyForm(e, index)}>
                            {replyButtonTexts[index] || "답글"}
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="replyCommentForm border-3 rounded-lg p-3 mt-4 mb-6 bg-white hidden">
                        <div className="font-bold text-lg">{loggedInNickname}</div>
                        <form onSubmit={(e) => handleReplySubmit(e, replyButtonTexts[index], index)}>
                          <div className="relative">
                            {/* 원글의 작성자 */}
                            <input type="hidden" name="id" defaultValue={post.id} />
                            {/* 답글 대상자 username */}
